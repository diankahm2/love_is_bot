...
EDITING, EDITING_PHOTO,block_check, EDITING_TEXT, SEND_CHECK, send_check_to_admin, ASK_PHOTO, GENDER, PHOTO, SURNAME, RU, HEIGHT, AGE, SPECIALITY, INCOME, INSTA, BIO, CHECK, SEARCHING, SEARCHING1, SEND_PHOTO, ADMIN_CHECKING, ADMIN_CHECKING1 = range(23)
...
def check(update: Update, context: CallbackContext) -> int:
    ...
    database.add_chat_gender(update.message.chat_id,user_data["gender"], user_data["surname"])
...
def searching(update: Update, context: CallbackContext) -> int:
    if update.message.text == 'Следующий':
        ...
        reply_keyboard_2 = [['Взаимность с '+str(update.message.chat_id), 'Игнор с '+str(update.message.chat_id)]]
        if context.user_data["gender"] == 'Мужчина':
        ...
        print("update mss", update.message.text)
        #update.message.reply_text('У вас взаимность с ' + database.match(context.user_data["surname"]) + 'можете написать этому аккаунту')
        return SEARCHING1
        print(update.message.text)

    elif ('Взаимность' in update.message.text):
        user = update.message.from_user
        chat_id = update.message.text[12:]
        #chat_id = update.message.text.split("")[len(update.message.text)-1]

        print("CHAT ID", chat_id)
        print("CHAT ID енннзу", type(chat_id))
        updater.bot.send_message(text='Вам ответили в заимностью '+ '@'+str(user.username) + ' можете написать', chat_id=int(chat_id))
        #updater.bot.send_message(text='Вам ответили взаимностью '+ '@'+str(update.message.chat_id) + ' можете написать', chat_id=int(update.message.chat_id))
        updater.bot.send_message(text='Вам ответили взаимностью c прекрасной Дианой', chat_id=int(update.message.chat_id))

        username_a = database.get_username_by_chat_id(chat_id)

        print("uuuuseeer", username_a)
        updater.bot.send_message(text='Вам ответили взаимностью '+ '@'+str(username_a[0]) + ' можете написать', chat_id=int(update.message.chat_id))
    ...
    elif update.message.text[:5] == 'Игнор':
        print("Игнор", update.message.text[:5])
        user = update.message.from_user
        chat_id = update.message.text[8:]
        updater.bot.send_message(text='Вам проигнорили '+ user.first_name + ' что там еще', chat_id=chat_id)
        reply_keyboard = [['Назад', 'Like', 'Следующий']]
...
def searching1(update: Update, context: CallbackContext) -> int:
    if update.message.text == 'Следующий':
        ...
        if context.user_data["gender"] == 'Мужчина':
            update.message.reply_text('Вы лайкнули пользовятеля '+ users_female[context.user_data["viewer"]][4] + 'что там еще')
            print("likeddd ffffff", context.user_data["surname"], users_female[context.user_data["viewer"]][4])
            #database.like(context.user_data["surname"], users_female[context.user_data["viewer"]][4])

            print("кому отправить сообщение что его лайкнули ", users_female[context.user_data["viewer"]][4])

            chat_id_towrite = int(database.send_liked_message_female(users_female[context.user_data["viewer"]][4])[0])
            print("chatt idd fuck", chat_id_towrite)
            updater.bot.send_message(text="Вас лайкнул %s можете его тоже лайкнуть" % (context.user_data["surname"]), chat_id=chat_id_towrite)
            reply_keyboard = [['Назад', 'Like', 'Следующий']]
            print("MESSAGE TO", chat_id_towrite)
            updater.bot.send_photo(chat_id=chat_id_towrite, reply_markup=ReplyKeyboardMarkup(reply_keyboard_2, one_time_keyboard=True), photo=open(context.user_data["path_to_photo"], 'rb'), caption='\n2. '+context.user_data["surname"]+'\n3. '+context.user_data["age"]+'\n4. '+context.user_data["bio"]+'\n5. '+context.user_data["ru"]+'\n6. '+context.user_data["income"]+'\n7. '+context.user_data["surname"]+'\n8. '+context.user_data["surname"]+'\n9. '+context.user_data["surname"]+'\n10. '+context.user_data["surname"] )
        ...
        chat_id = update.message.text[12:] #камила чат айди
        #chat_id = update.message.text.split("")[len(update.message.text)-1]
        print("Chat id", chat_id)                               #  shaha user     kamila chat_id
        updater.bot.send_message(text='Вам ответили взаимностью aaa '+ '@'+str(user.username) + ' можете написать', chat_id=int(chat_id))

        #шазмару написать что его лайкнула камила
        print("heeeyyyyyyy", database.get_username_by_chat_id("425471123"))

        print("heeeyyyyyyy 2222", database.get_username_by_chat_id(chat_id))

        username = database.get_username_by_chat_id(str(chat_id))[0]

        print("uuuuseeer", username)
        updater.bot.send_message(text='Вам ответили взаимностью '+ '@'+str(username) + ' можете написать', chat_id=int(update.message.chat_id))
...
def block(update: Update, context: CallbackContext) -> int:
    print("hhhggf", context.args)
    chat_id="140340457"
    user = update.message.from_user
    reply_keyboard = [['Заблокировать '+str(context.args[0])], ['Не блокировать '+ str(context.args[0])]]
    updater.bot.send_message(text=user["username"]+str("хочет заблокировать ")+str(context.args[0]), chat_id=chat_id, reply_markup=ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True))
    return block_check

def block_check(update: Update, context: CallbackContext) -> int:
    print("here", update.message.text)
    if "Заблокировать" in update.message.text:
        print('Нужно блокнуть')
    else:
        print("не блокать")
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        #entry_points=[CommandHandler('send_photo', send_photo)],
        states={
            ...
            block_check: [MessageHandler(Filters.text & ~Filters.command, block_check)],
