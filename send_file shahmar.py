...
users = []
#users_female = []
#users_male = [(1, 'Мужчина', 'path to photo', 'ruslan', 'ruslan', 'tt', 'tt', 'tt', 'tt', 'tt', 'tt', 'rr', '425471123', '0'), (2, 'Мужчина', 'path to photo', 'hh', 'hh', 'tt', 'gg', 'ff', 'uu', 'jj', 'ii', 'kk', '425471123', '0'), (3, 'Мужчина', 'path to photo', 'll', 'll', 'jj', 'kk', 'kk', 'jj', 'jj', 'kk', 'kk', '425471123', '0')]
...
def check(update: Update, context: CallbackContext) -> int:
    ...
    database.add(user_data["gender"], photo_path, user_data["age"], user_data["surname"], user_data["surname"], user_data["ru"], user_data["height"], user_data["speciality"], user_data["income"], user_data["insta"], user_data["bio"], update.message.chat_id)
    ...
    if update.message.text == 'Правильно': #kogda najmet' pravil'no nujno napisat' if exist potomu chto redaktirovanie toje zdes' proidet tak chot nujno napisat' kak to
        ...
        reply_keyboard = [['Назад', 'Like', 'Следующий', 'Редактировать профиль']]
        ...
        if context.user_data["gender"] == 'Мужчина':
            ...
            print("bred",users_male[context.user_data["viewer"]][1])
...
def searching(update: Update, context: CallbackContext) -> int:
    if update.message.text == 'Следующий':
        ...
        print("type", type(context.user_data))

        context.user_data = context.user_data + 1
        reply_keyboard = [['Назад', 'Like', 'Следующий', 'Редактировать профиль']]
        ...
        update.message.reply_text('Вы лайкнули пользовятеля '+ update.message.from_user + 'что там еще')
        database.like(update.message.from_user, users_female[context.user_data["viewer"]][3])
...
def searching1(update: Update, context: CallbackContext) -> int:
    if update.message.text == 'Следующий':
        ...
        update.message.reply_text('Вы лайкнули пользовятеля '+ update.message.from_user + 'что там еще')
        database.like(update.message.from_user, users_male[context.user_data["viewer"]][3])
        # zdes' update.message.from_user likenul female[context.user_data["review"]][3]
        reply_keyboard = [['Назад', 'Like', 'Следующий', 'Редактировать профиль']]
